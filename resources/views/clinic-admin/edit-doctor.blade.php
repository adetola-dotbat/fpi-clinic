        ...
        <main class="page-content">
            <div class="row">
                <div class="col-xl-9 mx-auto">
                    ...
                    <div class="card">
                        <div class="card-body">
                            <form action="{{ route('doctor.update', $doctor_edit->id) }}" method="POST">
                                @csrf
                                @method('post')
                                ...
                                @endif
                                <input class="form-control mb-3" type="text" value="{{ $doctor_edit->name }}"
                                    name="name" placeholder="Full Name">
                                @if ($errors->has('name'))
                                    <p class="text-danger">{{ $errors->first('name') }}</p>
                                @endif
                                <input class="form-control mb-3" type="email" value="{{ $doctor_edit->email }}"
                                    name="email" placeholder="Email">
                                @if ($errors->has('email'))
                                    <p class="text-danger">{{ $errors->first('email') }}</p>
                                @endif
                                <input class="form-control mb-3" type="phone" value="{{ $doctor_edit->phone }}"
                                ...
                                <select name="department_id" class="form-select mb-3" required>
                                    @foreach ($departments as $department)
                                        <option value="{{ $department->id }}"
                                            {{ $doctor_edit->department_id == $department->id ? 'selected' : '' }}>
